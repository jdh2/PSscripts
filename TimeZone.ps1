...
[CmdletBinding()]
param(
    [string]$IpStackAPIKey = "a3ec79306b8ea2e263f7a4f192f9c498",
    
    [string]$BingMapsAPIKey = "AuWMfUDJPlH5zrbbXl4yA0c7GGN_lnU8XdIiXs455p-X8abunxwLcDW8kisWXPBC" ,
...
Try{
    If(-not(Test-Path "$RegHive\SOFTWARE\TimezoneSelector") ){
        New-Item -Path "$RegHive\SOFTWARE" -nAME "TimezoneSelector" -ErrorAction Stop -Verbose | Out-Null       
    }
}
Catch{
...
ElseIf((Get-ItemProperty "$RegHive\SOFTWARE\TimezoneSelector" -Name Status -ErrorAction SilentlyContinue).Status -eq "Running"){
...
ElseIf($TimeSelectorRunOnce){
    ...
    If(-not((Get-ItemProperty "$RegHive\SOFTWARE\TimezoneSelector" -Name Status -ErrorAction SilentlyContinue).Status -eq "Completed") ){
